...
We've already discussed Internet Protocols and domain names.  The file path portion of the URL can also provide valuable information about the server. It reads exactly like a Unix file path on the command line.  The path "/species/common_birds.htm" indicates that the 

### Dynamic Files

In the above example, when you use the url https://sfbaywildlife.info/species/common_birds.htm to request a resource the secure server at associated with the sfbaywildlife.info server siply finds the file located on the server at ./species/common_birds.html and delivers it directly to your web browser.  We call this a static web server, because the server itself does not do any processing of files prior to delivery.  It simply receives requests for files living on the server and then sends them to the client, whose browser renders the file for viewing.
...
Dynamic websites, such as e-commerce sites that are connected to databases, require a mechanism for users to subit information to the server for processing.  This is accomplished through one of two protocols:  GET or POST.
