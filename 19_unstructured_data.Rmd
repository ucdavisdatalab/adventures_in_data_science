...
With our metadata structured, it's time to load our files and see what we can 
do with them.
...
For the purposes of analyzing this text data, the best that can be said is that, 
...
to do more work to **preprocess** our texts before we can analyze them.
...
an alphabetic character or a space:

```{r}
cleaned <- lapply(files, function(x) str_replace_all(x, "[^A-Za-z]", " "))
```
...
And here are the first nine words:

```{r}
frankenstein[[1]][1:9]
```

This representation of text is called a **bag of words**.
...
```{r}
list(untokenized=frankenstein[[1]][1:9], tokenized=tokenized[[6]][[1]][1:5])
```
...
```{r}
frankenstein_bigrams <- tokenize_ngrams(files[[6]], n=2, stopwords=stopwords("SMART"))
```

Here, `n=2` sets the n-gram window at two:

```{r}
head(frankenstein_bigrams[[1]], 30)
```
...
two words together into a **base form** by **lemmatizing** them. Though this 
...
```{r echo=FALSE}
rm(cleaned, tokenized, drac, dracula, wuthering_heights, wh, frankenstein, 
   frankenstein_bigrams, comparison_words, i)
```

### Working with the Text Mining Package
...
A `tm` corpus works somewhat like a database. It has a section for "content," 
...
```{r}
str_sub(corpus[[10]]$content, 1, 1000)
```
...
Oftentimes, this preprocessing can take a while, so now would be a good place 
to save your data. That way, you can retrieve your corpus later on.

```{r eval=FALSE}
saveRDS(corpus, "./data/C19_novels_cleaned.rds")
```

Loading it back in is straightforward:

```{r}
corpus <- readRDS("./data/C19_novels_cleaned.rds")
```
...
a **tf--idf** score (short for "term frequency---inverse document frequency"). 
...
There's a bit more semantic focus in these results, especially for "boat". And 
...
is because only a few novels talk about boats.
...
seem to be particular to each novel.
