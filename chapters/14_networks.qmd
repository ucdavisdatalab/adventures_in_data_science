...
::: {.callout-important}
The examples in this chapter use the following data sets:
...
net_url1 = "../../images/ch14/male-teacher_emoji.png"
net_url2 = "../../images/ch14/female-student_emoji.png"
net_url3 = "../../images/ch14/female-health-worker_emoji.png"
net_url4 = "../../images/ch14/male-scientist_emoji.png"
...
as.data.frame(as.matrix(
    network(head(toy_edgelist, n = 10))
))
```
...
# Plot
visNetwork(nodes = .vul_att, edges = .vul_edge) %>%
  visInteraction(zoomView = FALSE)
```
...
# Plot
visNetwork(nodes = .vul_att[.vul_att$color != "green", ], edges = .vul_edge) %>%
  visInteraction(zoomView = FALSE)
```
...
class_edges = read.csv(
  "data/toy_projected.csv", header = TRUE, stringsAsFactors = FALSE
)
class_attributes = read.csv(
  "data/toy_attributes.csv", header = TRUE, stringsAsFactors = FALSE
)
...
# Plot!
visNetwork(nodes = .projected_df, edges = class_edges) %>%
  visInteraction(zoomView = FALSE)
```
...
```{r echo=FALSE}
visNetwork(toy_attributes, toy_edgelist, main = "High Density") %>%
  visInteraction(zoomView = FALSE)
```

```{r echo=FALSE}
visNetwork(
  toy_attributes, toy_edgelist[c(1:5, 7, 10, 13, 15), 1:2],
  main = "Low Density"
) %>% visInteraction(zoomView = FALSE)
```
...
.star_edge = data.frame("from" = 1:5, "to" = rep(6, 5))
...
.loop_edge = data.frame("from" = 1:6, "to" = c(2:6, 1))
