...
@sec-implicit-coercion explains why this happens.
<!-- FIXME , and @sec-more-about-comparisons explains several other ways to
compare values. -->
:::
...
## Example: CA Least Terns

Let's use R to read the California least tern data set introduced in
@sec-ca-least-terns.
...
The data set is in a file called is `2000-2023_ca_least_tern.csv`, which
suggests it's a CSV file. The function to read a CSV file is `read.csv`. The
function's first and only required argument is the path to the CSV file.
...
## Inspecting a Data Frame {#sec-inspecting-a-data-frame}
