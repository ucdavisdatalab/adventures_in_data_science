...
A **parser** converts structured data into familiar data structures. The
[rvest][] package is designed to parse HTML. It can scrape HTML tables
automatically and supports two different ways to select tags: XPath and CSS
selectors (both described in subsequent sections).

[rvest]: https://rvest.tidyverse.org/

Let's get acquainted with rvest by parsing the example of a complete web page
from earlier. The rvest function `read_html` reads an HTML document. It accepts
either an HTML string or a file path (including URLs):
...
The `html_children` function returns all of the immediate children of a given
element.
...
The result from `html_children` is a **node set** (`xml_nodeset` object). Think
...
The `html_text` function returns the text contained in a tag. Let's get the text
...
:::{.callout-note}
Sometimes you might need to parse XML rather than HTML. R has two popular
packages for parsing XML:
...
The XML package has more features, most of which are related to writing (rather
than parsing) XML documents.

The xml2 package is more user-friendly, and as part of the Tidyverse, it's
relatively well-documented. The rvest package is based on the xml2 package.
:::


## XPath

An HTML document is a tree, similar to the file system on your computer:
...
**XPath** is a language for writing paths to elements in an HTML document. XPath
...
The rvest function `html_elements` finds all elements at given XPath:
...
```{r}
html_elements(doc, xpath = "//*[contains(text(), 'paragraph')]")
```
...
The rvest package uses CSS selectors by default, which is why we've so far had to specify that we are using XPath selectors.

Here are a few examples of CSS selectors, using rvest's `html_elements` function:
...
# Get all links
html_elements(doc, css = "a")

# Get all tags with id="hello"
html_elements(doc, "#hello")
```
...
In R, rvest's `read_html` function takes care of step 1 for you, although you can
...
```{r}
tables = html_elements(wiki_doc, xpath = "//table")
tables
```
...
```{r}
tab = html_elements(tables, xpath = "//*[contains(@class, 'sortable')]")
tab
```
...
**Step 3** is to clean up the data frame. The column names contain symbols and
the column types are not correct.
...
How can we clean up the `date` column? Recall from @sec-date-processing that
the lubridate package provides a variety of functions for parsing strings into
dates. In this case, we need a function that parses month, day, and year, which
means the `mdy` function. Here's the code to convert the dates in the data
frame:
...
One of the strings fails to parse. We can use `is.na` to find out which one:

```{r}
cities$date[is.na(dates)]
```

It looks like this date has a footnote after it. We can fix this with the
stringr package. We'll use `str_split_fixed` to split the dates on `[` and keep
only the first part:

```{r}
library("stringr")

dates = str_split_fixed(cities$date, fixed("["), 2)
dates = dates[, 1]
cities$date = mdy(dates)
```

We should also convert the population to a number. Again we need to remove
footnotes. This time we'll use a different approach:
...
Now the data frame is quite a bit cleaner:
...
```{r}
html_elements(doc, xpath = "//div[contains(@class, 'td_block_inner')]")
# OR html_elements(doc, "div.td-block-inner")
```

That returns a lot of results, so let's try using the `id` attribute, which is
`"tdi_73"`, instead. Usually the id of an element is unique, so this ensures
that we get the right section.

We can also add in a part about getting links now:

```{r}
div = html_elements(doc, xpath = "//div[@id = 'tdi_73']")
# OR html_elements(doc, "div#tdi_73")

links = html_elements(div, xpath = ".//a")
# OR html_elements(div, "a")
...
```{r}
links = html_elements(div, xpath = ".//h3/a")
# OR html_elements(div, "h3 > a")
...
```{r}
feature_urls = html_attr(links, "href")
```
...
parse_article_links = function(page) {
  div = html_elements(page, xpath = "//div[@id = 'tdi_73']")
  links = html_elements(div, xpath = ".//h3/a")
...
```{r}
nav = html_elements(doc, xpath = "//div[contains(@class, 'page-nav')]")
# OR html_elements(doc, "div.page-nav")
next_page = html_elements(nav, xpath = ".//a[contains(@aria-label, 'next-page')]")
# OR html_elements(nav, "a[aria-label *= 'next-page']")
```
...
parse_article_links = function(page) {
  ...
  # Get next page URL
  nav = html_elements(page, xpath = "//div[contains(@class, 'page-nav')]")
  next_page = html_elements(nav, xpath = ".//a[contains(@aria-label, 'next-page')]")
  next_url = html_attr(next_page, "href")
...
the `html_elements` function returns an empty node set for the next page URL, so
there aren't any `href` fields for `html_attr` to extract. It's convenient that
the rvest functions behave this way, but we could also add an if-statement to
