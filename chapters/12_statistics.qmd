...
library("remotes")
install_github("speegled/fosdata")
```
:::
...
```{r annotated-density-tails, echo=FALSE, cache=TRUE}
# Set z to -4, 4
z = seq(-4, 4, length.out = 1000)

# Plot the standard normal density function with some annotations
plot(
  x = z, y = dnorm(z), bty = 'n', type = 'l', main = "Standard Normal Density"
)

# Annotate the density function with the 5% probability mass tails
xs = c(z[z <= qnorm(0.025)], qnorm(0.025), min(z))
ys = c(dnorm(z[z <= qnorm(0.025)]), 0, 0)
polygon(x = xs, y = ys, col = grey(0.8))

xs = c(z[z >= qnorm(0.975)], max(z), qnorm(0.975))
ys = c(dnorm(z[z >= qnorm(0.975)]), 0, 0)
polygon(x = xs, y = ys, col = grey(0.8))
```
...
```{r exponential-density, echo=FALSE, cache=TRUE}
...
of waist measurement is to increase BMI by `r round(coef(fit2)[[2]], 2)` and an
