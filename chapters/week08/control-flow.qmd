...
As an illustrative example, suppose we want our code to produce a different
greeting depending on the hour. If it's the morning (hours 6-11), the code
should produce `"Good morning!"`. If it's the afternoon (hours 12-17), the code
should produce `"Good afternoon!"`. For any other hour, the code should produce
`"Hello."`. We can use a conditional to do this (see @sec-logic if you need a
refresher on how to write conditions that return `TRUE` and `FALSE` values):
...
As a flowchart, the control flow looks like this:

```{mermaid}
flowchart LR
  start(Start) --> any_more{Any more datasets?}
    any_more -- Yes --> read["read_mass_data()"]
    read --> any_more
    any_more -- No --> done(End)

style read font-family:monospace
```
:::
