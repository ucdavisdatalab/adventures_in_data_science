...
The "If Statement" is the most basic of the R control stuctures.  It simply tests whether a particular condition is truee.  For example, the below statement test wheter the value of the variable *x* is greater than 5.  If it is, the code print the phrase "Yay!" to screen.  If it is not, the code doesn nothing:
...
Note, the general syntax in the example:

```
control_statement (condition) {
     #code to exectute condition is true
}
```

While you will occasionaly see variations in how control structures are present, this is a fairly universal syntax across computer programming languages.  The specific control structure being invoked is followed by the *condition* to be tested and any actions to be peformed if the condition evaluates to TRUE are place betyween curly branckets {} following the condition.
...
Another common task is to test whether a BOOLEAN value is TRUE or FALSE.  The sybtax for this evaluation is simply:

```
if (*x*) { #do something}
```

Control structures in R also have a negation symbol which allows you to specifify a negative condition.  For example, the conditional statement in the follow code evaluates to TRUE (meaning any code placed between the curly brakets wikl be executed) if the x IS NOT EQUAL to 5:
...
The If Else statement is similar to the If statement but it allows you specify one code path to execute if the conditional evaluates to TRUE and antother to execute if the conditional evaluated to FALSE:
...
R also offers a simple, combined if/else syntax for quick execution of small code chunks:
...
The *switch* statement provides a simpe mechanism for selecting between multiple possible conditions. For example, the following code returns one of several possible values from a list based upon the value of a variable:
...
## The *whitch* Statementy ##

The *witch* statement is not true conditional statement, but it provides a very useful way to testing the values of a dataset and telling you which elements match a particular condition.  In the example below, we load the R IRIS dataset and find out wich rows have a Petal.Length greater than 1.4:
