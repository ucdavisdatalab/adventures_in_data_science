...
Section \@ref(inspecting-data) and Section \@ref(inspecting-the-data)
...
The `as.Date` function converts times and dates to R's `Date` class. This data
type allows us to do computations on dates, such as sorting by date or finding
...
Chapter \@ref(reshaping-tabular-data) gives examples of tidy and untidy data,
as well as explanations of how to make untidy data tidy.
...
of the feature. Recall from Section \@ref(data-types-classes) that the types
...
The `table` function, which was introduced in Section \@ref(inspecting-data),
...
1.  **Standard Deviation** - square root of the average squared distance to the
    mean. You can think of this as approximately the average distance from a
    data point to the mean. As a rule of thumb, most of the data will be within
...
2.  **Interquartile Range** (IQR) - difference between the 75th and 25th
    ...
    **Quantile** is another word for percentile. **Quartile** specifically
    refers to the 25th, 50th, and 75th percentiles.
...
have to do a bit more than just call `class(cl)`.
...
suppose we want to compute the medians for all of the columns in the earnings
